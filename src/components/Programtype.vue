  ...
  export default {
    ...
    data() {
      return {
        ...
        pagesize: 5, //每页的数据条数
        currentPage: 1, //默认
        protype: {ptid:"",ptname: ""}, //添加时提交的表单数据
        url: "",
        rule: {
          ptname: [{
            required: true,
    ...
    methods: {
      ...
      addtype: function() {
        this.$refs["fm"].validate(vlid => {
          if (vlid) {
            this.$axios.post("backstage/programtypeinfo/addprotype", {
              "ptname": this.protype.ptname,"ptid":this.protype.ptid
            }).then(response => {
              if (response.data == 1) {
                this.protype.ptname = "";
                this.$message({
      ...
      delBytid: function(row) {
        this.$confirm("是否删除？", "提示", {
          ...
          this.$axios.post("backstage/programtypeinfo/deleteprotype", {
            "ptid": row.ptid
          }).then(response => {
            if (response.data == 1) {
              this.$message({

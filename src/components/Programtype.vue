<template>
    <div >
      ...
      <el-form label-suffix="：" ref="fm"  :model="protype"  :rules="rule" class="form">
        <el-form-item label="类型名称" prop="ptname">
          <el-input style="width: 200px;" v-model="protype.ptname"></el-input>
      ...
      <el-table :data="list"  width="50%" height="500px" :stripe="true" border>
        ...
        <el-table-column prop="ptname" label="节目类型"></el-table-column>
    ...
    export default {
        ...
        data(){
         return{
           ...
           pagesize: 5, //每页的数据条数
           currentPage: 1, //默认
           protype:{ptname:""},//添加时提交的表单数据
           url:"",
           rule:{
             ptname:[
               {required:true,message:"不能为空",trigger:'blur'},
               {required: true, pattern: /^[\u4e00-\u9fa5]+$/, message: '只能输入汉字', trigger: 'blur'}
             ]
           },
         }
        },created:function () {
          ...
          queryProgramtype:function () {
              this.$axios.post("backstage/programtypeinfo/selectType",{
        ...
        addtype:function () {
            this.$refs["fm"].validate(vlid=>{
              if(vlid){
                this.$axios.post("backstage/programtypeinfo/addprotype",{"ptname":this.protype.ptname}).then(response=>{
                  if(response.data==1){
                    this.protype.ptname="";
                    this.$message({
                      type: 'success',
        ...
        delBytid:function (row) {
          this.$confirm("是否删除？","提示",{
            ...
            type:"warning"
              ...
              this.$axios.post("backstage/programtypeinfo/deleteprotype",{"ptid":row.ptid}).then(response=>{
                if(response.data==1){
                  this.$message({
                    type:'info',
                    message:"已删除"
